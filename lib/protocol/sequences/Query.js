...
Query.prototype._handleFinalResultPacket = function(packet) {
  ...
  var args = [this._loadError], results = this._results, fields = this._fields;

  if(!results.length)
    args.push(packet);
  else {
    args.push(results.length > 1 ? results : results[0]),
    args.push(fields.length > 1 ? fields : fields[0]);
    args.push(packet);
  }
