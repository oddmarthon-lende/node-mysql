...
function Query(options, callback) {
  ...
  this.castMap = ConnectionConfig.parseCastMap(options.castMap);
...
Query.prototype['EofPacket'] = function(packet) {
  ...
  if (this._callback) {
    this._resultSet._parent = this._results;
    this._results.push(this._resultSet);
  }
...
Query.prototype._finish = function(err) {

  var args = [(err ? err : this._loadError)];

  for(var i = 0, len = this._results.length, ok, rs = this._results[i]; i < len; i++, rs = this._results[i]) {
    
    if(rs instanceof Packets.OkPacket) {

      ok = rs;

      if(!i || this._results[i - 1] instanceof Packets.OkPacket) {
        rs = new ResultSet();
        rs.serverInfo = ok;
        this._results[i] = rs;
        i--;
...
Query.prototype['RowDataPacket'] = function(packet, parser) {
  packet.parse(parser, this._resultSet.fieldPackets, this.castMap, this.nestTables);

  if (this._callback) {
    this._resultSet.push(packet);
  } else {
    this.emit('result', packet, this._index);
  }
};
